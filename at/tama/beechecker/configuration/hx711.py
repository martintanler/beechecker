...
class HX711:
    ...
    def calc_reference_unit(self, callback:None, expected_weight, times=10):
        val = 0
        for i in range(times):
            val = val + self.get_value(3)
            if callback is not None:
                callback(i)
            time.sleep(0.5)
            self.reset()
        return float(float(val/times)/float(expected_weight))
    ...
    def calc_offset(self, callback=None, times=15):
        value = 0
        for i in range(times):
            value = value + self.read_average(3);
            if callback is not None:
                callback(i)
            time.sleep(0.5)
        return value/times
